...
from common.custom_types import STAGE_FIVE, STAGE_FOUR, STAGE_ONE, STAGE_SIX, STAGE_THREE, STAGE_TWO, TemplateType
...
from stage_1_validation.custom_types import ContestantSubmission, StageOneInput, StageOneOutput
...
from stage_2_ranking.custom_types import Musicosa as S2_Musicosa
...
from stage_3_templates_pre_gen.custom_types import Musicosa as S3_Musicosa
...
from stage_3_templates_pre_gen.stage_input import load_avatars_from_db
...
from stage_6_video_gen.custom_types import EntryVideoOptions, StageSixInput, StageSixOutput, Timestamp
        ...
        *,
        ...
        def adapted_data_collector(config: Config) -> D:
            ...
            if len(inspect.getfullargspec(data_collector).args) == 0:
                ...
                raise RuntimeError(
                    f"Data collector function '{data_collector}' takes more than 1 arguments "
        ...
        while True:
            ...
            try:
                ...
                if ask_to_continue_on_success:
                    choice = better_input(
                        on_success_msg, lambda x: x in controls, error_message=lambda x: f"Invalid choice '{x}'"
                    )
            except Exception as error:
                print(f"{err_header} {error}" if err_header else error)
                choice = better_input(
                    on_error_msg, lambda x: x in error_controls, error_message=lambda x: f"Invalid choice '{x}'"
                )
...
def retry_or_reconfig[O](
    config_loader: ConfigLoader, err_header: str | None = None
) -> Callable[[RetryReconfigPipelineStep[O]], RetryReconfigPipelineStep[O]]:
    decorator = step_executor_decorator(
        controls={RETRY, RELOAD_CONFIG, ABORT}, err_header=err_header, config_loader=config_loader
    )
...
def configless_stage[D, O](
    data_collector: DataCollector[D], err_header: str | None = None
) -> Callable[[ConfiglessStagePipelineStep[D, O]], ConfiglessStagePipelineStep[D, O]]:
    decorator = step_executor_decorator(
        controls={CONTINUE, CONTINUE_AND_SKIP, RETRY, RELOAD_DATA, ABORT},
        err_header=err_header,
        data_collector=data_collector,
    )
...
def stage[D, O](
    config_loader: ConfigLoader, data_collector: DataCollector[D], err_header: str | None = None
) -> Callable[[PipelineStep[D, O]], PipelineStep[D, O]]:
...
class PipelineStateManager:
    ...
    def register_submissions(self, submissions: list[ContestantSubmission]) -> None:
                ...
                if entry.is_author:
                    ...
                    if entry.video_timestamp:
                        ...
                        self.video_options.append(
                            VideoOptions(
                                entry=new_entry, timestamp_start=parse_time(start), timestamp_end=parse_time(end)
                            )
                        )
        ...
        for sub in submissions:
            for entry in sub.entries:
                self.scoring_entries.append(
                    Scoring(
                        contestant=self.contestants_by_name[sub.name],
    ...
    def register_stage_2_output(self, stage_output: StageTwoOutput) -> None:
        self.contestant_stats_collection.extend(
            [
                ContestantStats(
                    contestant=self.contestants_by_name[stat.contestant.name],
        ...
        self.entry_stats_collection.extend(
            [
                EntryStats(
                    entry=self.entries_by_title[stat.entry.title],
    ...
    def produce_stage_3_input(self) -> StageThreeInput:
        ...
        entries_index_of_unfulfilled_templates: dict[int, Entry] = dict(
            [(stat.ranking_sequence, stat.entry) for stat in self.entry_stats_collection]
        )  # pyright: ignore [reportAssignmentType]

        entry_ids_with_video_options = [options.entry.id for options in self.video_options]
        entries_index_of_unfulfilled_video_options: dict[int, Entry] = dict(
            [
                (stat.ranking_sequence, stat.entry)
        ...
        return StageThreeInput(
            S3_Musicosa(
                unfulfilled_contestants,
                self.avatars,
                entries_index_of_unfulfilled_templates,
        ...
        for entry in self.entries:
            ...
            entries_video_options.append(
                EntryVideoOptions(
                    ...
                    timestamp=Timestamp(start=str(video_options.timestamp_start), end=str(video_options.timestamp_end)),
...
if __name__ == "__main__":
    ...
    def stage_1_collect_input(config: Config) -> StageOneInput:
        submissions = get_submissions_from_forms_folder(
            config.stage_1.forms_folder, config.stage_1.contestant_name_coords, config.stage_1.entries_data_coords
        )
    ...
    @stage(
        err_header="[Stage 5 | Execution ERROR]",
        config_loader=configuration_loader,
    ...
    @stage(
        err_header="[Stage 6 | Execution ERROR]",
        config_loader=configuration_loader,
