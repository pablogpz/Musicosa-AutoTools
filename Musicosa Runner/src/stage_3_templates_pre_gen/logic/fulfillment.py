...
from common.time.utils import parse_time, time_str_zfill, validate_video_timestamp_str
...
def fulfill_unfulfilled_avatar_pairings(
    unfulfilled_contestants: list[Contestant], available_avatars: list[Avatar]
) -> list[AvatarPairing]:
    pairings: list[AvatarPairing] = []

    print("")
    print(".: AVATAR PAIRINGS :.")
    print("")
    ...
    for contestant in unfulfilled_contestants:
        ...
        if len(available_avatars) > 0:
            choice = better_input(
                ...
                error_message=lambda x: f"Invalid option '{x}' (Pick a valid option number or '{choice_new_avatar}' to create a new avatar)",
            )
        else:
        ...
        if choice == choice_new_avatar:
            ...
            score_top = better_input(
                "Score box relative top position (%)",
            ...
            score_left = better_input(
                "Score box relative left position (%)",
                lambda x: re.match(r"^-?\d{1,3}([.]\d+)?$", x) is not None,
            ...
            score_font_scale = better_input(
                "Score font scale (factor)",
            ...
            score_font_color = better_input(
                "Score font color (CSS color)", default=last_score_font_color, indent_level=1
            )
            last_score_font_color = score_font_color

            avatar = Avatar.Insert(
...
def fulfill_unfulfilled_templates(entries_sequence_number_index: dict[int, Entry]) -> list[Template]:
    ...
    def parse_selection(selection_str: str) -> list[int]:
        return (
            parse_sequence_selection_of_kvstore(selection_str, entries_sequence_number_index, get_missing_templates())
            or []
        )
    ...
    while len(templates) != len(entries_sequence_number_index):
        ...
        author_avatar_scale = better_input(
            "Author's avatar scale (factor)",
            lambda x: re.match(r"^\d+([.]\d+)?$", x) is not None,
        ...
        for sequence_number in selection:
            templates[sequence_number] = Template(
                entry=entries_sequence_number_index[sequence_number],
...
def fulfill_unfulfilled_generation_settings() -> list[Setting]:
    ...
    if not is_setting_set(SettingKeys.GENERATION_VIDEOCLIPS_OVERRIDE_TOP_N_DURATION):
        ...
        override_top_n_videoclips = better_input(
            "Override duration of top-N videoclips (0=disabled)",
            lambda x: x.isdigit() and int(x) >= 0,
            error_message=lambda x: f"Invalid value '{x}' (Must be 0 or a positive number)",
            indent_level=1,
        )

        generation_settings.append(
            Setting(
                group_key=SettingGroupKeys.GENERATION,
    ...
    if not is_setting_set(SettingKeys.GENERATION_VIDEOCLIPS_OVERRIDE_DURATION_UP_TO_X_SECONDS):
        ...
        override_duration_value = better_input(
            f"Duration override (seconds) of top-N videoclips ({VIDEOCLIPS_OVERRIDE_DURATION_LIMIT}=full duration)",
            lambda x: (x.isdigit() and int(x) > 0) or x == str(VIDEOCLIPS_OVERRIDE_DURATION_LIMIT),
            error_message=lambda x: f"Invalid duration '{x}' "
...
def fulfill_unfulfilled_video_options(entries_sequence_number_index: dict[int, Entry]) -> list[VideoOptions]:
    ...
    def parse_selection(selection_str: str) -> list[int]:
        return (
            parse_sequence_selection_of_kvstore(selection_str, entries_sequence_number_index, get_missing_options())
            or []
        )
    ...
    while len(video_options) != len(entries_sequence_number_index):
        ...
        selection = parse_selection(
            better_input(
                "Options selection (seq_num|[start]:[end]|empty to select all remaining)",
                validate_selection,
                error_message=lambda x: f"Invalid selection '{x}' (Use a valid index, range, omit a boundary or leave empty)",
                indent_level=1,
            )
        ...
        video_timestamp = better_input(
            "Videoclip segment ([HH:]MM:SS)-([HH:]MM:SS)",
            lambda x: validate_video_timestamp_str(x, video_target_duration),
            default=last_video_timestamp,
            error_message=lambda x: f"Invalid segment '{x}' "
        ...
        start_time, end_time = (
            parse_time(time_str_zfill(video_timestamp_bits[0])),
            parse_time(time_str_zfill(video_timestamp_bits[1])),
        )

        for sequence_number in selection:
            video_options[sequence_number] = VideoOptions(
                entry=entries_sequence_number_index[sequence_number], timestamp_start=start_time, timestamp_end=end_time
            )
