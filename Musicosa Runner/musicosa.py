...
class StageOneConfig:
    ...
    def __init__(self, csv_forms_folder: str = DEFAULT_AWARD_FORMS_FOLDER):
        csv_forms_folder = csv_forms_folder.strip()

        self.award_forms_folder = csv_forms_folder.removesuffix("/") if csv_forms_folder.endswith("/") \
...
if __name__ == '__main__':
    ...
    def stage_2_collect_input(*, _: Config) -> StageTwoInput:
    ...
    def stage_3_collect_input(*, _: Config) -> StageThreeInput:
    ...
    def stage_4_collect_input(*, config: Config) -> StageFourInput:
        ...
        return StageFourInput(templates_api_url=config.stage_4.api_url,
                              artifacts_folder=config.artifacts_folder,
                              templates=[S4Template(template.nomination.id, slugify(f"{template.nomination.award}-"
                                                                                    f"{template.nomination.game_title}-"
                                                                                    f"{template.nomination.nominee}"))
    ...
    def stage_5_collect_input(*, _: Config) -> StageFiveInput:
    ...
    def stage_6_collect_input(*, _: Config) -> StageSixInput:
