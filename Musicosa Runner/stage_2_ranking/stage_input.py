...
from stage_2_ranking.custom_types import Musicosa, Contestant as S2_Contestant, Score as S2_Score, \
    Entry as S2_Entry
...
def load_musicosa_from_db() -> Musicosa:
    ...
    for contestant in contestants:
        ...
        s2_scores: list[S2_Score] = [S2_Score(entry_title=scoring.entry.title, value=scoring.score)
                                     for scoring in scoring_entries]

        s2_contestants.append(S2_Contestant(name=contestant.name, scores=s2_scores))

    entries: list[Entry] = [entry.to_domain() for entry in Entry.ORM.select()]
    s2_entries: list[S2_Entry] = [S2_Entry(title=entry.title, author_name=entry.author.name) for entry in entries]
