...
def validate_contestant_submission_entry(entry: ContestantSubmissionEntry,
...
def validate_contestant_submission(submission: ContestantSubmission,
    ...
    entries_count = contestants_count * rounds_count
    validation_errors: list[str] = []

    if len(entries) != entries_count:
        validation_errors.append(
            f"Submission entries count mismatch ({len(entries)}) (Should be {entries_count})")
    ...
    for entry in entries:
        if entry_errors := validate_contestant_submission_entry(entry, special_entry_topics):
...
def validate_contestant_submission_collection(submissions: list[ContestantSubmission],
                                              valid_titles: list[str],
                                              special_entry_topics: list[SpecialEntryTopic] | None) -> list[str] | None:
    contestants_count = len(submissions)
    rounds_count = get_setting_by_key("globals.rounds_count").value
    validation_errors: list[str] = []

    if len(submissions) != contestants_count:
        validation_errors.append(
            f"Submission forms count mismatch ({len(submissions)}) (Should be {contestants_count})")

    # Validate that each entry has exactly one author across all submissions

    authors_registry: dict[str, set[str]] = {}

    for sub in submissions:
        contestant_name, entries = sub.name, sub.entries
        for entry in entries:
            title, is_author = entry.title, entry.is_author
            if is_author and title in authors_registry:
                authors_registry[title].add(contestant_name)
            elif is_author:
                authors_registry[title] = {contestant_name}  # Careful: Using 'set()' here would make a set of chars
            elif title not in authors_registry:
                authors_registry[title] = set()

    if entries_no_author := [item[0] for item in authors_registry.items() if len(item[1]) == 0]:
        validation_errors.append(f"Entries with no authors ({len(entries_no_author)}): "
                                 f"{', '.join([f"'{title}'" for title in entries_no_author])}")

    if entries_multiple_authors := [item for item in authors_registry.items() if len(item[1]) > 1]:
    ...
    for sub in submissions:
        if submission_errors := validate_contestant_submission(sub, contestants_count, rounds_count, valid_titles,
