...
export type ResolvedScoring = Omit<Scoring, 'contestant' | 'entry' | 'estrelli'> & { formattedScore: string }

export type ResolvedContestant = Omit<Contestant, 'avatar'> & { avatar: ResolvedAvatar; scoring: ResolvedScoring }

export type ResolvedTemplateProps = Pick<
    Template,
...
function resolveContestant(
    baseContestant: Contestant,
    avatar: Avatar,
    scoring: Scoring,
...
export async function resolveTemplateProps(templateId: string): Promise<ResolvedTemplateProps | undefined> {
    ...
    const author = allContestants.find((contestant) => contestant.id === entry.author) ?? defaultAuthor
    const authorAvatar = avatars.find((avatar) => avatar.id === author.avatar) ?? defaultAvatar
    const authorScoring = scoringEntries.find((scoring) => scoring.contestant === author.id) ?? defaultScoring
