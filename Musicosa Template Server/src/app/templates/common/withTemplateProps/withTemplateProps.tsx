...
export default async function withTemplateProps<P extends object>(
    ...
    const scoreMinValueSetting =
        await settingsRepository.getSettingByKey<number>('validation.score_min_value')
    const scoreMaxValueSetting =
        await settingsRepository.getSettingByKey<number>('validation.score_max_value')

    return (props: P) =>
        <Component scoreMinValue={scoreMinValueSetting?.value ?? defaultTemplateSettingsProps.scoreMinValue}
