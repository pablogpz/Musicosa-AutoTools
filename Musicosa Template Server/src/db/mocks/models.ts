...
export const mockTemplate = (partial?: Partial<Template>): Template => ({
    ...
    authorAvatarScale: partial?.authorAvatarScale ?? chance.floating({ min: 0.1, max: 1 }),
...
export const mockContestantStats = (partial?: Partial<ContestantStats>): ContestantStats => ({
    contestant: partial?.contestant ?? chance.guid({ version: 4 }),
    avgGivenScore: partial?.avgGivenScore ?? chance.floating({ min: 0, max: 10 }),
    avgReceivedScore: partial?.avgReceivedScore ?? chance.floating({ min: 0, max: 10 }),
})

export const mockContestantStatsCollection = (n: number, partial?: Partial<ContestantStats>): ContestantStats[] =>
...
export const mockEntryStatsCollection = (n: number, partial?: Partial<EntryStats>): EntryStats[] =>
