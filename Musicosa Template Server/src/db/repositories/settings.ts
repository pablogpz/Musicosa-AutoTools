...
const parseSettingResult = (setting: RawSetting): Setting => ({
    ...setting,
    value: setting.type === 'integer' ? (setting?.value ? parseInt(setting.value as string) : null) :
        setting.type === 'real' ? (setting?.value ? parseFloat(setting.value as string) : null) :
            setting.type === 'boolean' ? (setting?.value ? Boolean((setting.value as string).toLowerCase()) : null) :
                setting?.value ?? null
} as Setting)

const getSettingByKey =
    async <V extends SettingValueType>(settingKey: SettingKeys): Promise<TypedSetting<V> | undefined> => {
        if (!settingsCache.has(settingKey) || process.env.NODE_ENV === "development") {
            const [settingGroupKey, settingName] =
                settingKey.split(SETTING_KEY_SEPARATOR) as [SettingGroupKeys, SettingNames]
